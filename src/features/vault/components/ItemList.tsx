...
export default function ItemList({ onSelect, onNew, onLock }: Props) {
  ...
  const { items, loading } = useVaultItems({ search, item_type: typeFilter })
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-b border-white/10 px-3 py-2">
        <span className="text-sm font-semibold text-gray-200">
          Bóveda {items.length > 0 && <span className="text-xs text-gray-500">({items.length})</span>}
        </span>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 p-8 text-center">
            <p className="text-xs text-gray-500">
              {search || typeFilter ? 'Sin resultados.' : 'No hay elementos en la bóveda.'}
            </p>
            {!search && !typeFilter && (
